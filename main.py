...
def create_background(cap, num_frames=30):
    ...
    if backgrounds:
        ...
        raise ValueError("could'nt capture any frames for background")

#create_mask creates a binary mask to identify a specific color range in the image
...
def main():
    ...
    #range for invisible cloak, mutable
    ...
    while True:
        ret, frame= cap.read()
        if not ret:
            print("error- could'nt read frame")
            time.sleep(1)
            continue
